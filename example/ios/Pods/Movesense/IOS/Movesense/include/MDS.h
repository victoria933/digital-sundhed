...
@optional
// Function that is called when error occurs during connection attempt.
// The reason is part of the error.
- (void)didFailToConnectWithError:(const NSError * _Nullable)error;
- (void)didFailToConnectWithErrorToUUID:(const NSError * _Nullable)error uuid:(const NSUUID * _Nullable)uuid;
@end
